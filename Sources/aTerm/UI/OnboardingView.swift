...
struct OnboardingView: View {
    ...
    @State private var selectedThemeID: String = "custom-default"
            ...
            HStack {
                ...
                if step < titles.count - 1 {
                    Button("Skip") {
                        ...
                        windowModel.onboardingDidComplete(selectedThemeID: selectedThemeID)
                    }
                    .controlSize(.large)
                }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let provider = appModel.provider(for: selectedPreset), provider.authType != .none {
                ...
                HStack(spacing: 12) {
                    ...
                    if let result = providerTestResult {
                        Text(result)
                            .font(.system(size: 12))
                            .foregroundColor(result.contains("ms") ? .green : .secondary)
                            .lineLimit(2)
                    }
                }
            ...
            if hasSavedCredential {
                HStack {
                    ...
                    Text("Credential saved. You can add more providers in Settings later.")
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                }
                .padding(.top, 4)
    ...
    private var shellIntegrationStep: some View {
        VStack(spacing: 14) {
            ...
            if let message = appModel.shellIntegrationMessage {
                Text(message)
                    .font(.system(size: 12))
                    .foregroundStyle(message.contains("Installed") ? .green : .secondary)
            }
